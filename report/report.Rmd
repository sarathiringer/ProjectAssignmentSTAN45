...
variable. Rendering $n$ to a total of $ 5278$ with no missing values. 
```{r desc, results='asis', echo=FALSE}

dfSummary(COMPAS, silent = TRUE, graph.col = FALSE, valid.col = FALSE)

# skim(compas) %>% select(-n_missing, -complete_rate) %>% kable(align = "r", digits = 2, booktabs = T, format = "latex") %>% 
...
data. Due to the nature of our analysis it might also be interesting to see
how well it performs in comparison to the other models fitted. Accuracy and
parity loss as measured by demographic parity for all models using unseen 
testing data can be seen in figure \@ref(fig:fap-test) and our chosen model
appears to have a satisfactory performance on test data as well, meeting the 80%
criterion as well as still maintaining a fairly high accuracy. 
