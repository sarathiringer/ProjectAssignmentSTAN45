...
ProPublica in 2016. The dataset, slightly modified from the original, is a
collection of $n = 6172$ observations with the variables: ` `r names(compas)` `.
Where `Two_yr_Recidivism` is the variable we want to predict and indicates
whether an inmate have made another offense within two years of release. As
such, the dataset is being used to decide on the penalty of criminals. The compas data is
an excellent case for studying discriminatory algorithms, as it both exhibits
bias and is being used for actual classification for a highly sensitive cause.
Because of this, many researchers who explore fairness in machine learning have
chosen to study it as an example. For the present purpose we filtered the data
on `Ethnicity`, for `Caucasian` and `African_American` to create a binary protected
variable.
...
fit on mitigated datasets and was therefore left out of the analysis.
...
that we can mitigate. However, all of the resampled models have a parity loss of less
than 0.2 (i.e. they satisfy the 80% rule) but the Random Forest models seems
to maintain accuracy best. Investigating the RF-models fitted on preferentially 
sampled data further, we can evaluate whether another cutoff than 
the conventional 0.5-value would yield even better results in terms of fairness.

```{r cutoff-rf, echo=FALSE, fig.cap="CUTOFF PLOT", out.width='100%',  results='hide', message=FALSE}

load("models/fobject_rfs.Rdata")
...
```
It can be seen in figure \@ref(fig:cutoff-rf) that parity loss as measured by
demographic parity would be minised with a cutoff at 0.42 instead of 0.5.
Equalised odds on the other hand is minised at 0.5. So depending on which measure
we want to use, we have to decide on a cutoff. If we add the preferentially resampled
Random Forest model with a cutoff at 0.42 to the other RF models we can evaluate whether
this drop in parity loss is worth any potential drop in accuracy. In figure \@ref(fig:fap-rf-comp)
it is clear that adjusting the cutoff removed all bias with regards to demographic
parity, but the big drop in accuracy is probably not worth it since the preferentially
resampled RF model with a cutoff at 0.5 is minimising equalised odds as well as keeping 
a fairly high accuracy.

```{r fap-rf-comp, echo=FALSE, fig.cap="RF comparison", out.width='100%',  results='hide'}

load("models/rf_comp.Rdata")

fap <- performance_and_fairness(rf_comp, fairness_metric = "STP")
plot(fap) +
  ggtitle("") +
  labs(x = "Accuracy", y = "Inversed parity loss (demographic parity)", color = "Model") 
