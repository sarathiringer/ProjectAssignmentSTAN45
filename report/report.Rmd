...
* Disparate impact removal 
    - Introduced in @feldman_certifying_2015. This is a geometric method reshaping 
      distributions on numerical variables conditioned on the protected. It is generally an apprioriate method when there are non-protected variables that are correlated with the protected variable, which means using them for prediction introduces disparate treatment by the model.
...
other non-sensitive attributes (källa [tror du verkligen det behövs?]). In this sense, penalizing variables that
...
Using demographic parity as a fairness measure
...
kable(data.frame(fap$paf_data), caption = "Model Performance and Fairness on Test Data",
      format = "latex", booktabs = T,
